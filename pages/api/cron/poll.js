...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ── Step 4: Record track_adds (bulk, skip existing) ──────────────────────
    try {
    ...
    // ── Step 8: Mark curators as polled ──────────────────────────────────────
    await sql`UPDATE curators SET last_polled_at = NOW() WHERE status = 'approved'`;

    const realErrors = results.errors.filter((e) => e.step || !e.error?.includes("network"));
    return res.status(200).json({ ok: realErrors.length === 0, ...results });
  } catch (err) {
