...
// ── Playlist tracks via metadata endpoint (avoids restricted /tracks endpoint) ─
// GET /playlists/{id} embeds the first page of tracks in its response.
// The dedicated /playlists/{id}/tracks endpoint requires extended quota;
// the metadata endpoint does not.
export async function fetchPlaylistTracks(playlistId, maxTracks = 100) {
  const token = await getReadToken();

  const fields = "tracks(items(added_at,is_local,track(id,name,artists,album,popularity,duration_ms,explicit,preview_url,external_urls,external_ids,uri)),next,total)";
  let res;
  try {
    res = await fetch(
  ...
  if (res.status === 429) {
    ...
    console.warn(`[spotify] rate-limited on playlist [${playlistId}], waiting ${retryAfter}s`);
