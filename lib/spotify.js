// ── Client-credentials token (for reads) ────────────────────────────────────
...
// ── User OAuth token (for playlist writes) ───────────────────────────────────
let userToken = null;
...
export async function getUserToken() {
  ...
  if (!refreshToken) throw new Error("SPOTIFY_REFRESH_TOKEN not configured — cannot write playlists");
  ...
  // Log if token rotated (Spotify sometimes issues a new refresh token)
  if (data.refresh_token && data.refresh_token !== refreshToken) {
    console.log("[spotify] Refresh token rotated — update SPOTIFY_REFRESH_TOKEN env var");
  }
  return userToken;
}
...
export async function discoverPlaylists() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      console.warn(`[spotify] featured-playlists ${res.status}: ${await res.text()}`);
    }
  } catch (e) {
...
// GET /search works in development mode for public track data.
// GET /playlists/{id}/tracks requires Extended Quota → 403.
export async function searchTracks(query, limit = 50) {
  const token = await getSpotifyToken();
  const params = new URLSearchParams({ q: query, type: "track", market: market(), limit: String(limit) });
  ...
  try {
    ...
    console.error(`[spotify] searchTracks network error (${query}):`, e.message);
    return [];
  }

  if (res.status === 429) {
    const wait = parseInt(res.headers.get("Retry-After") || "5", 10);
    console.warn(`[spotify] rate-limited on search, waiting ${wait}s`);
    await new Promise((r) => setTimeout(r, wait * 1000));
    return searchTracks(query, limit);
  }

  if (!res.ok) {
    console.error(`[spotify] searchTracks (${query}) failed: ${res.status}`);
    return [];
  }
...
export async function fetchPlaylistTracks(playlistId, maxTracks = 100) {
  ...
  if (res.status === 429) {
    ...
    console.warn(`[spotify] rate-limited fetching ${playlistId}, waiting ${retryAfter}s`);
    await new Promise((r) => setTimeout(r, retryAfter * 1000));
    return fetchPlaylistTracks(playlistId, maxTracks);
  }
...
// ── Batch-fetch full track objects (with popularity + ISRC etc.) ─────────────
...
// ── Spotify user ID (needed to create playlists) ─────────────────────────────
...
export async function createPlaylist(userId, name, description = "") {
  ...
  return res.json(); // returns playlist object with .id
}

// ── Replace playlist tracks (in safe chunks, retry on 429) ───────────────────
