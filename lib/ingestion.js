...
export async function ingestTrackObjects(tracks) {
  ...
  for (const [trackId, t] of trackMap.entries()) {
    ...
    // Spotify dev-mode omits popularity from search and GET /tracks responses.
    // Fall back to primary artist popularity as a proxy — it's correlated and available.
    const primaryArtist = artists[t.artists?.[0]?.id];
    const popularity = t.popularity || primaryArtist?.popularity || 0;
    ...
    const { rows } = await sql`
      INSERT INTO tracks (
        ...
        ${popularity}, ${t.preview_url || null},
        ${t.external_urls?.spotify || null}, ${image}, ${isrc},
        ${genres}, ${JSON.stringify(t)}, NOW()
      )
      ON CONFLICT (spotify_track_id) DO UPDATE SET
        popularity = GREATEST(EXCLUDED.popularity, tracks.popularity), preview_url = EXCLUDED.preview_url,
