...
export async function runMigrations() {
  ...
  await sql`ALTER TABLE track_adds ADD COLUMN IF NOT EXISTS popularity_current INTEGER`;
  await sql`ALTER TABLE track_adds ADD COLUMN IF NOT EXISTS popularity_refreshed_at TIMESTAMPTZ`;
  await sql`ALTER TABLE curators ADD COLUMN IF NOT EXISTS avg_popularity FLOAT DEFAULT 0`;
  await sql`ALTER TABLE curators ADD COLUMN IF NOT EXISTS predictive_lift FLOAT DEFAULT 0`;
